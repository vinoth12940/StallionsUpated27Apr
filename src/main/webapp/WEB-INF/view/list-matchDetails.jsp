...
<body>
    ...
    <div class="content-wrapper">
        <section class="content-header">
            <h1>Match <small>Match List</small></h1>
            <ol class="breadcrumb">
                <li>Home</li>
                <li>Match</li>
            </ol>
                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title">Result Report search</h3>
                        </div>
						<div class="box-header">
                            <security:authorize access="hasAnyRole('ADMIN')">
								...
								<input type="button" value="Add Match"
                            ...
                            <div class="table-responsive">
                                <table id="example2" class="table table-striped table-hover table-sm">
                                    ...
                                    <tbody>
										...
										<c:forEach var="tempMatch" items="${theMatchDetails}">
											<!-- construct an "update" link with customer id -->
											<c:url var="updateLink" value="/matchDetails/showFormForUpdate">
												<c:param name="matchId" value="${tempMatch.matchSK}" />
											</c:url>					
											<tr>
												<td> ${tempMatch.matchSK} </td>
												<td> ${tempMatch.match_Date} </td>
												<td>  
													<c:forEach var="tempTournamet" items="${tournamentNamelist}">
														<c:if test = "${tempTournamet.getTournamentSk() == tempMatch.getTournamentSk()}">
															<c:out value = "${tempTournamet.tournamentName}"/>
														</c:if>
													</c:forEach>		
												</td>
												<td> ${tempMatch.opponent_Team_Name} </td>
												<td> ${tempMatch.match_Type} </td>
												<td> ${tempMatch.match_Overs} Overs</td>
												<td> ${tempMatch.match_Result} </td>
												<security:authorize access="hasAnyRole('ADMIN')">
												<td>
													<security:authorize access="hasAnyRole('ADMIN')">
